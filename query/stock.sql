...
case when s.name isnull then d.name else s.name end as product_type,s.category,
...
from stock
left join(select serial_number,round(cost_price) as cost_price,carton_id from product_hashes)ph on ph.serial_number=true_serial
